...
export async function getSignedUrl(req: Request, res: Response) {
    try {
        ...
        if (!bucketName) {
            return res.status(500).json({ error: 'GCS bucket name is not configured' });
        }
        const fileContentType = contentType || 'application/octet-stream';
        const url = await generateV4UploadSignedUrl(bucketName, fileName, fileContentType);
    ...
    try {
        ...
        const uploadData = await Promise.all(
            files.map(async (file: { fileName: string; contentType?: string }) => {
                ...
                const signedUrl = await generateV4UploadSignedUrl(bucketName, file.fileName, fileContentType);
        ...
        res.status(500).json({ error: 'Failed to generate signed URLs', details: (error as Error).message });
    }
}
